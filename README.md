...
1. После бага сохраните игру на новый слот, закройте игру;
2. Скачайте [этот файл](https://github.com/ejetaxeblevich/XMLParser/releases/tag/260628a). В приложении релиза нажмите на `xmlparser.lua` - вы его скачаете;
3. Скачайте [этот файл](https://github.com/ejetaxeblevich/GameSave/releases/tag/260723a). В приложении релиза нажмите на `gamesave.lua` - вы его скачаете;
4. Откройте папку вашей установленной игры, например, у меня она лежит тут: `D:/Steam/steamapps/common/Hard Truck Apocalypse`;
5. Поместите ранее скачанные файлы внутрь папки `data`;
6. Откройте внутри игры файл `server.lua` по этому пути `data/scripts/server.lua` в любом текстовом редакторе, подойдет даже обычный Блокнот;
7. Пролистайте этот файл **в самый низ**. Добавьте следующий код:
```lua
XMLParser = require("data\\xmlparser.lua")
GS = require("data\\gamesave.lua")
```
8. Сохраните изменения в файле;
9. Теперь вам нужно выбрать, какие отношения группировок загрузить:

   9.1. Отношения группировок по умолчанию, когда начинается новая игра:

      9.1.1. Если вы выбрали этот вариант, перейдите к 10 пункту.
   
   9.2. Отношения группировок из ранее сделанного сохранения, когда они работали правильно:

      9.2.1. Вам нужно произвести поиск "хорошего" сохранения;
   
      9.2.2. Запустите игру;
   
      9.2.3. Перебирайте по очереди сохранения в игре, загружайте их по очереди от самого верхнего к нижнему и смотрите меню "Отношения";
   
      9.2.4. Если группировки в табличке отношений не одного цвета, **запомните имя слота этого сохранения**, например `Край Быстрое сохранение 2`;
   
      9.2.5. Откройте внутри игры папку `saves` по этому пути `data/profiles/имя_вашего_профиля/saves`;

      9.2.6. Найдите нужное сохранение:
   
      9.2.6.1. Если у вас есть программа для поиска значений по файлам, откройте эту папку в ней и вбейте в поиск **имя слота того сохранения**, который вы запомнили;
   
      9.2.6.2. Если у вас нет такой программы, вам придется поочередно раскрывать все сохранения в этой папке и искать вручную;
   
      9.2.7. Найдите **имя слота того сохранения**, который вы запомнили, внутри файла `saveinfo.xml` по пути сохранения `saves/номер_сохранения/saveinfo.xml`. Запомните путь `saves/номер_сохранения/` и добавьте после `maps/currentmap.xml`;
   
      9.2.8. У вас должен получиться путь, что-то наподобие `data/profiles/Player/saves/00000083/maps/currentmap.xml`. Запишите его. Замените в нем все `/` на `\\`.
   
11. Запустите игру, если она не запущена;
12. Загрузите ваше последнее сохранение, где отношения когда-то были сломаны;
13. Откройте внутриигровую консоль клавишей `~`. Если она не открывается и вы используете ***Community Mod Manager***, поставьте в нем галочку "Включить консоль" и перезапустите игру;
14. Скопируйте и выполните во внутриигровой консоли следующий код:

   13.1. Если вы решили загрузить отношения группировок по умолчанию:
```lua
GS:LoadToleranceStatus()
```
   13.2. Если вы нашли путь к нужному сохранению:
```lua
GS:LoadToleranceStatus("путь_к_файлу_сохранения_который_вы_нашли")
```
Например:
```lua
GS:LoadToleranceStatus("data\\profiles\\Player\\saves\\00000083\\maps\\currentmap.xml")
```
14. Проверьте группировки в меню "Отношения"!

> [!WARNING]
> Такой способ не лишен бага: Если вы играете в мод, который где-то запрашивает статус отношений, вы можете словить **софтлок**. В таком случае вы все сделали зря и придется загружать раннее сохранение :)

## Ниже инструкция для моддеров!
