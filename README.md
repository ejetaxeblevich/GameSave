...
9. Теперь вам нужно выбрать, какие отношения группировок загрузить:
   ...
   9.2. Отношения группировок из ранее сделанного сохранения, когда они работали правильно:
      ...
      9.2.7. Найдите **имя слота того сохранения**, который вы запомнили, внутри файла `saveinfo.xml` по пути сохранения `saves/номер_сохранения/saveinfo.xml`. **Запомните `номер_сохранения`**;
   
      9.2.8. У вас должен получиться путь, что-то наподобие `data/profiles/Player/saves/00000083`. Запишите его. Замените в нем все `/` на `\\`.
...
Например:
```lua
GS:LoadToleranceStatus("data\\profiles\\Player\\saves\\00000083")
```
